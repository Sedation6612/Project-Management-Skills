...
- If a conflict needs escalation, record it in `CONFLICTS.md` (repo cache) **and** write a reminder into `~/.openclaw/workspace/MEMORY.md` (single Markdown file) to ask the Master Project to resolve it during the next main-session check-in.
...
- If a project is created or updated, write a **concise reference** into `~/.openclaw/workspace/MEMORY.md` (single Markdown file) that points to the **verbose, detailed log entry** in this repo (e.g., `LOG.md` or `DECISIONS.md`).
